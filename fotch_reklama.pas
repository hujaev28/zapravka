...
procedure Totch_reklama.interval(int1,int2:Variant;massiv:Integer;z,w,s:String);
begin
...
procedure Totch_reklama.xturClick(Sender: TObject);
begin
  zapros.Text:='';
...
procedure Totch_reklama.xyilChange(Sender: TObject);
begin
  //dan.Text:='01.01.'+copy(xyil.Text,3,2);gacha.Text:='31.12.'+copy(xyil.Text,3,2);xturClick(Sender);
  xturClick(sender);
  end;

procedure Totch_reklama.FormActivate(Sender: TObject);
begin
...
procedure Totch_reklama.FormShow(Sender: TObject);
...
procedure Totch_reklama.FormHide(Sender: TObject);
var IniFile:TIniFile;
begin
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Totch_reklama.FormCreate(Sender: TObject);
var IniFile:TIniFile;
begin
// 201534642   200541303
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Totch_reklama.otch_old1Click(Sender: TObject);
...
procedure Totch_reklama.otch_newClick(Sender: TObject);
...
begin
  ...
  for i := 2 to 7 do begin if (sqltext.Text>'') and (poisk[i]>'') then
  poisk[i]:=' and '+poisk[i]; sqltext.Text:=sqltext.Text+poisk[i];end;
  with lbd_sf do begin
  Close;SQL.Clear;
  SQL.Add('SELECT  r.nom_uz as nom, COUNT(r.id) as soni, r.id as kesim from asos a,s_reklama r ');
  SQL.Add('WHERE a.client_id='+admin_id+' and r.id=a.reklama and a.del_flag=1 and '+sqltext.Text+' and a.reklama>0 group by r.nom_uz order by r.id'); // sf
  zapros.Text:=sql.Text;
  Open;
  if lbd_sf.RecordCount=0 then exit;
  SetLength(m, 0);col:=0;
  while not Eof do begin
      SetLength(m, Length(m)+1);
      m[col].i1:=Fieldbyname('soni').AsInteger;
      m[col].name:=Fieldbyname('nom').AsString;
      m[col].nn:=col;
      inc(col);
    Next;
  ...
  ExcelCreateApplication('Reklama', 1,2, false);
  try Sheet := Excel.WorkBooks[1].Sheets[1];except Exception.Create('Хато !!! Excel урнатилмаган !!!'); Exit;end;
  DisableControls;
  Sheet.Columns[1].ColumnWidth :=5;Sheet.Columns[2].ColumnWidth :=30;
  ...
  Sheet.Cells[firstRow-1,1] := '№';Sheet.Range['A'+IntToStr(firstrow-2)+':A'+IntToStr(firstrow-1)].merge;
  Sheet.Cells[firstRow-1,2] := 'Nomi';Sheet.Range['B'+IntToStr(firstrow-2)+':B'+IntToStr(firstrow-1)].merge;
  Sheet.Cells[firstRow-1,3] := 'Soni';Sheet.Range['C'+IntToStr(firstrow-2)+':C'+IntToStr(firstrow-1)].merge;
  ...
  Sheet.Cells[2,1]:='Reklama';Sheet.Range['A2:'+ustun+'2'].merge;Sheet.Cells[2,1].HorizontalAlignment := 3;
  Sheet.Cells[3,1]:=admin_fio;Sheet.Range['A3:'+ustun+'3'].merge;Sheet.Cells[3,1].HorizontalAlignment := 3;
  //Sheet.Cells[5,1]:=za.Text+' ('+zapros.Text+')';Sheet.Range['A5:'+ustun+'5'].merge;Sheet.Cells[5,1].HorizontalAlignment := 3;
  Excel.Visible := true;Excel := UnAssigned;
  end;
...
procedure Totch_reklama.otch_old2Click(Sender: TObject);
var firstRow,i,col,k,tkol_in:integer;s,ustun:string;bir:Double;
...
procedure Totch_reklama.foydaClick(Sender: TObject);
...
procedure Totch_reklama.chegirmaClick(Sender: TObject);
var firstRow,i,col:integer;s,ustun:string;
