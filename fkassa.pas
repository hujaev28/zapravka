...
begin
  ...
  if dms.asosid.AsInteger = 0 then
    Exit;
  ...
  if dms.asos_slavetovar_id.AsInteger = 0 then
    dms.asos_slavetovar_id.AsInteger := DEFAULT_TRANSPORT_TOVAR_ID;
  if dms.asos_slaveasos_id.AsInteger = 0 then
    dms.asos_slaveasos_id.AsInteger := dms.asosid.AsInteger;
  if (dms.asos_slaveturi.AsInteger = 0) and (turi_.ItemIndex = 0) then
    dms.asos_slaveturi.AsInteger := 1;
  if Trim(dms.asos_slavetovar_nom.AsString) = '' then
  begin
    if qoldiq.Active and (not qoldiq.IsEmpty) and (qoldiq.FindField('nom') <> nil) then
      dms.asos_slavetovar_nom.AsString := qoldiq.FieldByName('nom').AsString
...
begin
  ...
  dms.asos.Edit;
  dms.asosh_id.AsInteger:=0;
  dms.asoscheg_n.AsInteger:=0;dms.asoscheg_d.AsInteger:=0;
  dms.asossumma.AsInteger:=0;dms.asossum_d.AsInteger:=0;
...
procedure Tkassa.TransportFieldEnter(Sender: TObject);
begin
  EnsureTransportSlaveRow;
end;
...
begin
  ...
  sql.Close;
  sql.SQL.Clear;
  sql.SQL.Add('UPDATE asos set kurs = :kurs, summa = :summa, summa_ch = :summa_ch, sum_d_ch =:sum_d_ch where id = :id');
  ...
  if vSumma-dms.asoscheg_n.AsFloat<0 then begin
    sql.Params.ParamByName('summa_ch').AsFloat := 0;
    sql.Params.ParamByName('sum_d_ch').AsFloat := 0;
  end
  else
  begin
    sql.Params.ParamByName('summa_ch').AsFloat := vSumma-dms.asoscheg_n.AsFloat;
    sql.Params.ParamByName('sum_d_ch').AsFloat := vSumma-dms.asoscheg_d.AsFloat;
  end;
  sql.ExecSQL;
  dms.asos_slave.post;
  dms.asos.Refresh;
...
begin
  ...
  if jami.value-dms.asoscheg_n.AsFloat<0 then begin
    sql.Params.ParamByName('summa_ch').AsFloat:=0;
    sql.Params.ParamByName('sum_d_ch').AsFloat:=0;
  end
  else
  begin
    sql.Params.ParamByName('summa_ch').AsFloat := jami.value-dms.asoscheg_n.AsFloat;
    sql.Params.ParamByName('sum_d_ch').AsFloat := kub.value-DMS.asoscheg_d.AsFloat;
  end;
  sql.ExecSQL;
