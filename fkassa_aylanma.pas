...
procedure Tkassa_aylanma.interval(int1,int2:Variant;massiv:Integer;z,w,s:String);
begin
...
procedure Tkassa_aylanma.FormShow(Sender: TObject);
...
procedure Tkassa_aylanma.FormHide(Sender: TObject);
var IniFile:TIniFile;
begin
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Tkassa_aylanma.FormCreate(Sender: TObject);
var IniFile:TIniFile;
begin
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Tkassa_aylanma.g_asosDblClick(Sender: TObject);
begin
...
procedure Tkassa_aylanma.dancChange(Sender: TObject);
begin
  dan.Date:=danc.Date;
end;

procedure Tkassa_aylanma.gachacChange(Sender: TObject);
begin
  gacha.Date:=gachac.Date;
end;

procedure Tkassa_aylanma.danChange(Sender: TObject);
begin
  danc.Date:=dan.Date;
  hisoblaClick(sender);
end;

procedure Tkassa_aylanma.gachaChange(Sender: TObject);
begin
  gachac.Date:=gacha.Date;hisoblaClick(sender);
end;

procedure Tkassa_aylanma.RzBitBtn2Click(Sender: TObject);
...
begin
  ...
  for i:=0 to gpl.RecordCount-1 do begin
    ...
    begin
      if gplkonv.AsInteger >0 then begin
        if gplkonv.AsInteger = 1 then begin
          Sheet.Cells[firstrow+i,6]:=gplsena_pl.AsString;Sheet.Cells[firstrow+i,6].NumberFormat := '0,00';
        end;
        if gplkonv.AsInteger = 2 then begin
...
procedure Tkassa_aylanma.FormActivate(Sender: TObject);
begin
...
procedure Tkassa_aylanma.hisoblaClick(Sender: TObject);
var b,e,bd,ed:Currency;s:string;
begin
  ...
  gpl.SQL.Add('SELECT * FROM pl WHERE del_flag=1 and bank_id=0 and d_pl between '+kasaya(dan.Date)+' and '+kasaya(gacha.Date)+' order by d_pl');
  gpl.open;
end;
