...
procedure Totch_xarajat.interval(int1,int2:Variant;massiv:Integer;z,w,s:String);
begin
...
procedure Totch_xarajat.xturClick(Sender: TObject);
begin
  zapros.Text:='';
...
procedure Totch_xarajat.xyilChange(Sender: TObject);
begin
  //dan.Text:='01.01.'+copy(xyil.Text,3,2);gacha.Text:='31.12.'+copy(xyil.Text,3,2);xturClick(Sender);
  xturClick(sender);
  end;

procedure Totch_xarajat.FormActivate(Sender: TObject);
begin
...
procedure Totch_xarajat.FormShow(Sender: TObject);
...
procedure Totch_xarajat.FormHide(Sender: TObject);
var IniFile:TIniFile;
begin
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Totch_xarajat.FormCreate(Sender: TObject);
var IniFile:TIniFile;
begin
// 201534642   200541303
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Totch_xarajat.otch_old1Click(Sender: TObject);
...
procedure Totch_xarajat.otch_newClick(Sender: TObject);
var firstRow,i,col,tkol_in:integer;s,ustun:string;bir:Double;
...
begin
  ...
  zapros.Text:='';for i := 1 to 27 do poisk[i]:='';sqltext.Text:='';
  //if s_ter.Value>0 then begin poisk[2]:=' sf.ter = '+IntToStr(s_ter.Value);if s_ter.Value<91 then  zapros.Text:=zapros.Text+' Hududiy bo`lim = '+s_ter.Text+'#';end;
  if (dan.Text<>'  .  .  ') then interval(dan.Value, gacha.Value, 1,'d_pl' ,'d_pl' ,' d_pl ');
  if (poisk[1]>'') then sqltext.Text:=poisk[1];
  for i := 2 to 7 do begin if (sqltext.Text>'') and (poisk[i]>'') then poisk[i]:=' and '+poisk[i]; sqltext.Text:=sqltext.Text+poisk[i];end;
  with lbd_sf do begin
  Close;SQL.Clear;
  SQL.Add('SELECT x.nom as xnom,x.id as kesim,sum(pl.sena_pl) as summa,sum(pl.sena_d) as dollar from s_x_tur x,pl ');
  SQL.Add('WHERE x.id=pl.vo and pl.del_flag=1 and '+sqltext.Text+' group by xnom order by xnom'); // pl.client_id='+admin_id+' and
  //zapros.Text:=sql.Text;exit; 
  Open;if RecordCount=0 then exit;
  ExcelCreateApplication('Xarajatlar', 1,2, false);
  try Sheet := Excel.WorkBooks[1].Sheets[1];except Exception.Create('Хато !!! Excel урнатилмаган !!!'); Exit;end;
  DisableControls;
  Sheet.Columns[1].ColumnWidth :=5;Sheet.Columns[2].ColumnWidth :=30;
  Sheet.Columns[3].ColumnWidth := 8;Sheet.Columns[4].ColumnWidth :=6;Sheet.Columns[5].ColumnWidth := 12;
  ...
  Sheet.Columns[9].ColumnWidth := 8;Sheet.Columns[10].ColumnWidth :=6;Sheet.Columns[11].ColumnWidth :=10;
  Sheet.Columns[12].ColumnWidth := 8;Sheet.Columns[13].ColumnWidth :=8;Sheet.Columns[14].ColumnWidth :=10;
  firstRow:=10;progressl_lk.Percent:=0;col:=0;
  for i:=0 to RecordCount-1 do begin
    Sheet.Cells[firstrow+i,1]:=inttostr(i+1);inc(col);
    Sheet.Cells[firstrow+i,2]:=fieldbyname('xnom').AsString;s:=inttostr(firstrow+i);
    Sheet.Cells[firstrow+i,3]:=fieldbyname('summa').AsString;Sheet.Cells[firstrow+i,3].NumberFormat := '0,00';
    Sheet.Cells[firstrow+i,4]:=fieldbyname('dollar').AsString;Sheet.Cells[firstrow+i,4].NumberFormat := '0,00';
    Next;if RecordCount>1 then progressl_lk.Percent:=Trunc((i)/(RecordCount-1)*100);
  end;
  progressl_lk.Percent:=100;
  Sheet.Cells[firstRow-1,1] := '№';//Sheet.Range['A'+IntToStr(firstrow-2)+':A'+IntToStr(firstrow-1)].merge;
  Sheet.Cells[firstRow-1,2] := 'Xarajat turi';//Sheet.Range['B'+IntToStr(firstrow-2)+':B'+IntToStr(firstrow-1)].merge;
  Sheet.Cells[firstRow-1,3] := 'Summa';//Sheet.Range['C'+IntToStr(firstrow-2)+':E'+IntToStr(firstrow-2)].merge;
  Sheet.Cells[firstRow-1,4] := 'Dollar';//Sheet.Range['C'+IntToStr(firstrow-2)+':E'+IntToStr(firstrow-2)].merge;
  ...
  Sheet.Cells[2,1]:='Harajatlar';Sheet.Range['A2:'+ustun+'2'].merge;Sheet.Cells[2,1].HorizontalAlignment := 3;
  Sheet.Cells[3,1]:=admin_fio;Sheet.Range['A3:'+ustun+'3'].merge;Sheet.Cells[3,1].HorizontalAlignment := 3;
  Sheet.Cells[5,1]:=za.Text+' ('+zapros.Text+')';Sheet.Range['A5:'+ustun+'5'].merge;Sheet.Cells[5,1].HorizontalAlignment := 3;
  Excel.Visible := true;Excel := UnAssigned;
  end;
...
procedure Totch_xarajat.otch_old2Click(Sender: TObject);
...
procedure Totch_xarajat.foydaClick(Sender: TObject);
...
begin
  ...
  if StrToDate(dan.Text)>StrToDate(gacha.Text) then begin ShowMessage('Oraliq noto`g`ri');ActiveControl:=dan;Exit;end;
  zapros.Text:='';for i := 1 to 27 do poisk[i]:='';sqltext.Text:='';
  interval(dan.Value, gacha.Value, 1,'d_pl' ,'d_pl' ,' d_pl ');
  if (poisk[1]>'') then
  ...
  with lbd_sf do begin
  Close;SQL.Clear;
  SQL.Add('SELECT x.nom as xnom,pl.* from s_x_tur x,pl ');
  SQL.Add('WHERE x.id=pl.vo and pl.del_flag=1 and '+sqltext.Text+' order by d_pl'); // pl.client_id='+admin_id+' and
  //zapros.Text:=sql.Text;exit;
  Open;col:=0;
  if lbd_sf.RecordCount=0 then exit;
  ExcelCreateApplication('ro`yhat', 1,2, false);
  try Sheet := Excel.WorkBooks[1].Sheets[1];except Exception.Create('Хато !!! Excel урнатилмаган !!!'); Exit;end;
  DisableControls;
  Sheet.Columns[1].ColumnWidth :=5;Sheet.Columns[2].ColumnWidth :=10;Sheet.Columns[3].ColumnWidth := 15;
  Sheet.Columns[4].ColumnWidth :=15; Sheet.Columns[5].ColumnWidth :=30;
  firstRow:=10;progressl_lk.Percent:=0;col:=0;
  for i:=0 to RecordCount-1 do begin
    Sheet.Cells[firstrow+i,1]:=inttostr(i+1);inc(col);
    Sheet.Cells[firstrow+i,2]:=FieldByName('d_pl').AsString;
    Sheet.Cells[firstrow+i,3]:=FieldByName('sena_pl').AsString;Sheet.Cells[firstrow+i,3].NumberFormat := '0,00';
    Sheet.Cells[firstrow+i,4]:=FieldByName('sena_d').AsString;Sheet.Cells[firstrow+i,4].NumberFormat := '0,00';
    Sheet.Cells[firstrow+i,5]:=FieldByName('xnom').AsString;
    next;if RecordCount>1 then progressl_lk.Percent:=Trunc((i)/(RecordCount-1)*100);
  end;
  Sheet.Cells[firstRow-1,1] := '№';
  Sheet.Cells[firstRow-1,2] := 'Sana';
  Sheet.Cells[firstRow-1,3] := 'summa';
  Sheet.Cells[firstRow-1,4] := 'Dollar';
  Sheet.Cells[firstRow-1,5] := 'Xarajat turi';
  ...
  Sheet.Cells[2,1]:='Ro`yxat';Sheet.Range['A2:'+ustun+'2'].merge;Sheet.Cells[2,1].HorizontalAlignment := 3;
  Sheet.Cells[3,1]:=admin_fio;Sheet.Range['A3:'+ustun+'3'].merge;Sheet.Cells[3,1].HorizontalAlignment := 3;
  Sheet.Cells[5,1]:=za.Text+' ('+zapros.Text+')';Sheet.Range['A5:'+ustun+'5'].merge;Sheet.Cells[5,1].HorizontalAlignment := 3;
  Excel.Visible := true;Excel := UnAssigned;
  end;
