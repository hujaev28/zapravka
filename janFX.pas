...
{
 добавлено programania.com  15.05.2006
 procedure removeNoise(b:tBitmap; d,k:integer);
 procedure Sharpness(b:tBitmap; d,k:integer);
 procedure gamma(b:tBitmap; g:integer);
}
...
begin
   ...
   if YFinal<YOrigin then begin Memo:=YOrigin; YOrigin:=YFinal; YFinal:=Memo; end;  (* si diffВrence nВgative*)
...
if (xk>0) and (yk>0) and (xk<w-1) and (yk<h-1) then
     ...
     Vert:=(GG[1]+(GG[2]+GG[3]+GG[4]))div 4;           (* en cours d'Вvaluation        *)
...
//  fwidth:=ResampleFilters[ord(filter)-1].Width;
...
{$S-,R-}
PROCEDURE removeNoise(b:tBitmap; d,k:integer);
//k=100    обычное удаление шума
//k=-100..-800 усиление шума или резкости
//d= 1..4  радиус
var
x,y,z,xx,yy,j,i,n,q,x1,x2,x3,h,w,z1,z2,z3,r:integer;
m:array[1..120] of byte;
bi,bb: array of array of byte;
p0:pbytearray;

begin
h:=b.height;
w:=b.width;
setLength(bi,h+1,w+1);
setLength(bb,h+1,w+1);

// запись из bitmap b в массив bi
for y:=1 to h do begin
  p0:=b.scanline[y-1];
  x1:=0;x2:=1;x3:=2;
  for x:=1 to w do begin
    bi[y,x]:=(p0[x1]*28 + p0[x2]*151 + p0[x3]*76) shr 8;
...
// запись из массива bb в bitmap b разницы
for y:=1 to h do begin
  p0:=b.scanline[y-1];
  x1:=0; x2:=1; x3:=2;
  for x:=1 to w do begin

    r:=(bi[y,x]-bb[y,x])*k div 100;
    z1:=p0[x1]; dec(z1,r);
    z2:=p0[x2]; dec(z2,r);
    z3:=p0[x3]; dec(z3,r);

    if z1>255 then begin z:=z1-255; dec(z2,z); dec(z3,z); z1:=255 end;
    if z2>255 then begin z:=z2-255; dec(z1,z); dec(z3,z); z2:=255 end;
    if z3>255 then begin z:=z3-255; dec(z1,z); dec(z2,z); z3:=255 end;
...
// d  радиус
var
x,y,z,xx,yy,i,x1,x2,x3,h,w,z1,z2,z3,r,ss,q:integer;
m:array[1..120] of byte;
bi,bb: array of array of byte;
p0:pbytearray;
...
for y:=1 to h do for x:=1 to w do begin
  ...
  ss:=0;
  for yy:=y-d to y+d do for xx:=x-d to x+d do
     if (yy>=1)and(xx>=1)and(yy<=h)and(xx<=w) then begin
       inc(ss,bi[yy,xx]); inc(q);
     end;
  bb[y,x]:=ss div q;
end;

// запись из массива bb в bitmap b
for y:=1 to h do begin
  p0:=b.scanline[y-1];
  x1:=0; x2:=1; x3:=2;
  for x:=1 to w do begin
...
procedure gamma(b:tBitmap; g:integer);
// g=10..100..500  темнее..нормально..светлее
type
m255=array[0..255] of byte;
...
procedure Gamm(var gamma:m255;z:integer);
var i: integer;
begin
 if z=0 then z:=100;
...
for y:=0 to b.height-1 do begin
  p0:=b.scanline[y];
  xx:=0;
  for x:=1 to b.width do begin
    p0[xx]:=gamma[p0[xx]]; inc(xx);
    p0[xx]:=gamma[p0[xx]]; inc(xx);
    p0[xx]:=gamma[p0[xx]]; inc(xx);
  end;
end;
