...
type
  Tkirim = class(TForm)
    ...
    procedure Qayta_hisoblashClick(Sender: TObject);
...
var
  ...
  kirim: Tkirim; Forward,qidr,almash,Wrap,Replace:olevariant;
...
uses fs_client,fulanish, fdms, fs_diler, fs_ombor_edit, fkirim_asos,Math,clipbrd,
  ...
  fhato;
procedure Tkirim.RemakeQR(s:string);
// QR-code generation
begin
  with FQRCode do
  try
...
procedure Tkirim.FormShow(Sender: TObject);
...
procedure Tkirim.FormHide(Sender: TObject);
var IniFile:TIniFile;
begin
  IniFile := TIniFile.Create(ExtractFilePath(Application.ExeName)+'sozlash.ini');
...
procedure Tkirim.FormCreate(Sender: TObject);
...
procedure Tkirim.nom_shKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then begin Key:=#0;Perform(WM_NEXTDLGCTL,0,0);end;

end;

procedure Tkirim.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tkirim.anewClick(Sender: TObject);
begin
  tozalaClick(Sender);
  dms.k_asos.Append;
...
procedure Tkirim.aeditClick(Sender: TObject);
begin
  tozalaClick(Sender);
  dms.k_asos.edit;
...
procedure Tkirim.adelClick(Sender: TObject);
begin
  tozalaClick(Sender);
  dms.k_slave1.First;
...
procedure Tkirim.snewClick(Sender: TObject);
begin
  //dms.k_asos.Append;
  tozalaClick(Sender);
...
procedure Tkirim.seditClick(Sender: TObject);
begin
...
procedure Tkirim.sdelClick(Sender: TObject);
begin
  dms.main_link.Close;dms.main_link.SQL.Clear;
...
procedure Tkirim.TovarlarDblClick(Sender: TObject);
begin
...
procedure Tkirim.RzBitBtn1Click(Sender: TObject);
var f:Currency;
begin
  tozalaClick(Sender);
...
procedure Tkirim.g_asosCellMouseClick(Grid: TCustomGridEh;
...
procedure Tkirim.RzBitBtn2Click(Sender: TObject);
begin
...
procedure Tkirim.RzBitBtn3Click(Sender: TObject);
begin
  dms.s_tovar.Refresh;
  dms.asos.Close;dms.asos.SQL.Clear;
  dms.asos.SQL.Add('select * from asos where id='+dms.k_asos.fieldbyname('id').AsString);
  ...
  ulanish.frxsf.PrepareReport;
  ulanish.frxsf.PrintOptions.ShowDialog := false;
  ulanish.frxsf.ShowProgress := false;
  ulanish.frxsf.ShowReport;
end;

procedure Tkirim.Gs_kursExit(Sender: TObject);
begin
...
procedure Tkirim.Qayta_hisoblashClick(Sender: TObject);
var kurs_:Currency;
begin
  tozalaClick(Sender);
  ...
  while (not dms.k_asos.Eof) do begin //  and (dms.k_asosid.AsInteger>43705)
      ...
      while not dms.k_slave1.Eof do begin
        ...
        if (dms.k_asosdollar.AsFloat>0) then begin
          ...
          dms.k_slave1sotish_d.AsFloat:=RoundTo(dms.k_slave1sotish.AsFloat/kurs_,3);
          if dms.k_slave1sena_d.AsFloat=0 then
          dms.k_slave1sena_d.AsFloat:=RoundTo(dms.k_slave1sena.AsFloat/kurs_,3);
        end;
        dms.k_slave1.Post;
        if (dms.k_asosdollar.AsFloat>0) then begin
          dms.k_slave1.Edit;
          if dms.s_t_allkol_in.AsInteger>1 then begin
            if dms.k_slave1sena_in.AsFloat=0 then
            dms.k_slave1sena_in.AsFloat:=RoundTo(dms.k_slave1sena.AsFloat/dms.s_t_allkol_in.AsInteger,dms.s_clientyaxlitlash_in.Value);
          ...
          begin
            dms.k_slave1sotish_in.AsVariant:=null;dms.k_slave1sotish_in_d.AsVariant:=null;dms.k_slave1sena_in.AsVariant:=null;
            dms.k_slave1sena_in_d.AsVariant:=null;dms.k_slave1opt1_in.AsVariant:=null;dms.k_slave1opt1_pl_in.AsVariant:=null;
            dms.k_slave1opt2_in.AsVariant:=null;dms.k_slave1opt2_pl_in.AsVariant:=null;dms.k_slave1schet_in.AsVariant:=null;
          end;
          dms.k_slave1.Post;
        end;
        dms.k_slave1.Next;
...
procedure Tkirim.tozalaClick(Sender: TObject);
begin
...
procedure Tkirim.Edit1KeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
...
procedure Tkirim.FormActivate(Sender: TObject);
begin
...
procedure Tkirim.shtrixEnter(Sender: TObject);
begin
  shtrix.Text:='';
end;

procedure Tkirim.shtrixKeyPress(Sender: TObject; var Key: Char);
...
procedure Tkirim.newsfmClick(Sender: TObject);
begin
...
procedure Tkirim.tovarlar_komilKeyPress(Sender: TObject; var Key: Char);
begin
  if Key<>#13 then exit;
  seditClick(Sender);
...
procedure Tkirim.TovarlarKeyPress(Sender: TObject; var Key: Char);
begin
  if Key<>#13 then exit;
  seditClick(Sender);
end;

procedure Tkirim.tovarlar_komilKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tkirim.TovarlarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (ssCtrl in Shift) and (upcase(Char(key)) = 'C') then
    ClipBoard.AsText:= DMS.k_slave1s_tovar.AsString;
...
procedure Tkirim._asosck(Sender: TObject;
  var Handled: Boolean);
begin
  l_asos.Text:=''; 
end;

procedure Tkirim.qidirKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then begin
...
procedure Tkirim.tovarlar_komilDblClick(Sender: TObject);
begin
  if l_asos.Value>0 then
  begin
    if MessageDlg(asos.fieldbyname('nomer').AsString+' -xujjat raqamini o`zgartirasizmi ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
...
procedure Tkirim.X1Click(Sender: TObject);
begin
  Clipboard.AsText:=link.fieldbyname('serial').AsString;
end;

procedure Tkirim.linkAfterScroll(DataSet: TDataSet);
begin
  kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
  s_tovar_kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
end;

procedure Tkirim.linkAfterRefresh(DataSet: TDataSet);
begin
  kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
  s_tovar_kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
end;

procedure Tkirim.linkAfterPost(DataSet: TDataSet);
begin
  kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
  s_tovar_kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
end;

procedure Tkirim.linkAfterOpen(DataSet: TDataSet);
begin
  kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
  s_tovar_kirim.soni.Caption:=inttostr(kirim.link.RecordCount);
end;

procedure Tkirim.qidirEnter(Sender: TObject);
begin
  qidir.Text:='';

end;

procedure Tkirim.Label1Click(Sender: TObject);
begin
...
procedure Tkirim.dilerChange(Sender: TObject);
begin
...
procedure Tkirim.dilerEditButtons0Click(Sender: TObject;
  var Handled: Boolean);
begin
  diler.Value := null;
  Label1Click(Sender);
...
procedure Tkirim.genqrClick(Sender: TObject);
...
procedure Tkirim.cbbDrawingModeChange(Sender: TObject);
begin
...
procedure Tkirim.q1Click(Sender: TObject);
...
procedure Tkirim.docqrClick(Sender: TObject);
var c:char;qr:string;nn,i:integer;Bmp: TBitmap;Jpg: TJPEGImage;nomi,nomi_s,Shape,vstart, vend:OleVariant;
S, T : TFileStream;
begin
...
procedure Tkirim.Image2DblClick(Sender: TObject);
begin
...
procedure Tkirim.RzBitBtn5Click(Sender: TObject);
begin
  if l_asos.Value=0 then
  begin
...
procedure Tkirim.testClick(Sender: TObject);
    ...
    if dms.k_asosdollar.AsFloat=1 then begin
      ...
      while not dms.k_slave1.Eof do begin
        if (dms.k_slave1sena_d.AsFloat>=dms.k_slave1sotish_d.AsFloat) then begin
          dms.k_slave1.Edit;
          d:=RoundTo(dms.k_slave1sena_d.AsFloat*dms.k_slave1foiz.AsFloat/100,-1);
          dms.k_slave1sotish_d.AsFloat:=dms.k_slave1sena_d.AsFloat+d;
...
procedure Tkirim.RzBitBtn7Click(Sender: TObject);
begin
  exit;
  {dms.k_asos.First;
  while not dms.k_asos.Eof do begin
    if dms.k_slave1.RecordCount=0 then
      dms.k_asos.Delete;
    else
      dms.k_asos.Next;
  end;
...
procedure Tkirim.RzBitBtn8Click(Sender: TObject);
begin
  dms.s_tovar.Refresh;
  dms.asos.Close;dms.asos.SQL.Clear;
  dms.asos.SQL.Add('select * from asos where id='+dms.k_asos.fieldbyname('id').AsString);
  ...
  begin
    if dms.s_clientdollar.AsInteger=1 then
      ulanish.frxsf.LoadFromFile(ExtractFilePath(Application.ExeName)+'sf_kirim_k_s.fr3')
    else
      ulanish.frxsf.LoadFromFile(ExtractFilePath(Application.ExeName)+'sf_kirim_d_s.fr3');
  end
  else
    ulanish.frxsf.LoadFromFile(ExtractFilePath(Application.ExeName)+'sf_kirim_s.fr3');
  ulanish.frxsf.PrintOptions.Printer := setup.cbPrinters.Text;
  ulanish.frxsf.SelectPrinter;
  ulanish.frxsf.PrepareReport;
  ulanish.frxsf.PrintOptions.ShowDialog := false;
