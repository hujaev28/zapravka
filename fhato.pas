...
var
  hatolar: Thatolar;
  kirim_s,kirim_d,sotish,sotish_d,sotish_in,sotish_in_d,ulg1,ulg1_pl,ulg2,ulg2_pl,schet:Double;
...
begin
  ...
  DMS.tombor.FieldByName('dnom').AsString:=dms.link.fieldbyname('dnom').AsString;
  ...
  if (dms.s_clientqoldiq.AsInteger=1) and (hatolar.s_t_all.fieldbyname('qoldiq').AsString='Y') then begin
    k:=Trunc(DMS.tombor.FieldByName('kir_all').AsInteger / hatolar.s_t_allkol_in.AsInteger); // 20
    k_in:=DMS.tombor.FieldByName('kir_all').AsInteger - k*hatolar.s_t_allkol_in.AsInteger;  // 0
    DMS.tombor.FieldByName('kol').AsInteger:=k;
    DMS.tombor.FieldByName('kol_in').AsInteger:=k_in;
    DMS.tombor.FieldByName('kol_all').AsInteger:=k*hatolar.s_t_allkol_in.AsInteger+k_in;
...
procedure Thatolar.Button1Click(Sender: TObject);
begin
...
procedure Thatolar.Button2Click(Sender: TObject);
begin
  if Edit1.Text<>'123456' then exit;
  dms.main_link.Close;dms.main_link.SQL.Clear;
...
procedure Thatolar.testClick(Sender: TObject);
var kam,sot,limit:integer;s_t_id:string;
begin
  Memohato.Visible:=true;Memohato.Lines.Clear;Memohato.Refresh;
  Memohato.Lines.Add('Tovarlarning qoldiqlarini qayta hisoblash jarayoni boshlandi');
  Memohato.Lines.Add('Iltimos kutib turing !!!');
  ...
  while not s_t_all.Eof do begin
    ...
    if s_t_allqoldiq.AsString='Y' then begin
      ...
      if kam-sot<>limit then begin
        Memohato.Visible:=true;
        Memohato.Lines.add('Xatolik:  '+s_t_allid.AsString+' '+s_t_allnom.AsString+' = '+IntToStr(kam-sot)+' <> '+inttostr(limit));
        //exit;
      end;
    end;
    end
    ...
    begin
      if (s_t_allholat.AsInteger=0) and (dms.tombor.Locate('tovar_id',s_t_allid.AsInteger,[loCaseInsensitive])) then begin
        Memohato.Visible:=true;
        Memohato.Lines.add(' 1/Xatolik:  '+s_t_allid.AsString+' '+s_t_allnom.AsString);
        //exit;
      end;
    end;
    s_t_all.Next;
  end;
  Memohato.Lines.Add('Jarayon tugadi, sabr uchun rahmat  !!!');
end;

procedure Thatolar.qoldiq_tekshirClick(Sender: TObject);
var kam,sot,limit,k,k_in:integer;s_t_id:string;
begin
  if t_id.Text='' then begin
    if MessageDlg('Hamma qoldiqlar qayta hisoblanadi , oxirigacha sabr qilasizmi ?',mtConfirmation, [mbYes, mbNo], 0) = mrNo then exit;
  end;
  Memohato.Visible:=true;Memohato.Lines.Clear;Memohato.Refresh;
  Memohato.Lines.Add('Tovarlarning qoldiqlarini qayta hisoblash jarayoni boshlandi');
  Memohato.Lines.Add('Iltimos kutib turing !!!');
  if t_id.Text<>'-' then begin
    s_t_all.Close;s_t_all.SQL.Clear;
   if t_id.Text='site' then s_t_id:=' upakavka>0 and ' else s_t_id:=' id='+t_id.Text+' and ';
   if t_id.Text='' then s_t_id:='';
  ...
  while not s_t_all.Eof do begin
    ...
    if t_id.Text<>'' then begin
      dms.main_link.Close;dms.main_link.SQL.Clear;
      dms.main_link.SQL.Add('delete from ombor WHERE tovar_id='+s_t_allid.AsString);
      dms.main_link.ExecSQL;dms.tombor.Refresh;
    end;
    dms.main_link.Close;dms.main_link.SQL.Clear;
    dms.main_link.SQL.Add('select sum(a.kol) as kol_1,sum(a.kol_in) as kol_in_1 from s_tovar t,asos,asos_slave a');
    dms.main_link.SQL.Add('WHERE a.del_flag=1 and asos.tur_oper in (2,4) and a.asos_id=asos.id and t.id=a.tovar_id and a.tovar_id='+s_t_allid.AsString+' group by a.tovar_id order by a.tovar_id');
    dms.main_link.open;
    sot:=dms.main_link.fieldbyname('kol_1').AsInteger*s_t_allkol_in.AsInteger+dms.main_link.fieldbyname('kol_in_1').AsInteger;

    dms.link.Close;dms.link.SQL.Clear;
    dms.link.SQL.Add('select a.id,a.tovar_id,a.polka,a.seriya,a.srok,a.kol,a.kol_in,a.sena,a.sena_d,a.sotish,a.sotish_in,a.sotish_d,a.sotish_in_d,a.opt1,a.opt1_pl,a.opt2,a.opt2_pl,a.schet, ');
    dms.link.SQL.Add('d.nom as dnom,t.kol_in as t_in,t.nom as nom,t.nom_sh as nom_sh,asos.sana,s_kat.nom as knom,asos.dollar as kesak,asos.kurs as kesakd from asos_slave a,asos left JOIN s_diler d ON d.id=asos.diler_id,s_tovar t left JOIN s_kat ON s_kat.id = t.kat ');
    dms.link.SQL.Add('WHERE a.del_flag=1 and asos.tur_oper in (1,3,5) and a.asos_id=asos.id and t.id=a.tovar_id and a.tovar_id='+s_t_allid.AsString+' order by a.id desc');
    //link.SQL.Add('select a.id,d.nom as dnom,t.kol_in as t_in,t.nom as nom,t.nom_sh as nom_sh,asos.sana,s_kat.nom as knom,asos.dollar as kesak,asos.kurs as kesakd from asos_slave a,asos left JOIN s_diler d ON d.id=asos.diler_id,s_tovar t left JOIN s_kat ON s_kat.id = t.kat ');
    //Memohato.Visible:=True;
    //Memohato.Lines.Add(dms.link.SQL.Text);exit;
    dms.link.open;
    ...
    begin
      if dms.link.RecordCount>0 then begin
        ...
        if (sot>limit) and (s_t_allqoldiq.AsString='Y') then begin
          Memohato.Visible:=true; Memohato.Lines.add('Xatolik '+s_t_allid.AsString+' '+s_t_allnom.AsString+' - '+inttostr(sot)+'>'+inttostr(limit));
    ...
    s_t_all.Edit;
    s_t_allupakavka.AsVariant:=null;
    s_t_all.post;
  ...
  dms.main_link.ExecSQL;
  dms.tombor.Refresh;
  hatolar.FormActivate(Sender);
  Memohato.Lines.Add('Jarayon tugadi, sabr uchun rahmat  !!!');
end;

procedure Thatolar.SiteClick(Sender: TObject);
begin
  //4780030391625
  t_id.Text:='site';
  qoldiq_tekshirClick(Sender);
end;

procedure Thatolar.FormActivate(Sender: TObject);
begin
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('select count(id) as jami from s_tovar WHERE upakavka>0 and del_flag=1 and holat=1');
  dms.main_link.open;

  Site.Caption:='site = '+dms.main_link.fieldbyname('jami').AsString;
end;

procedure Thatolar.RzBitBtn1Click(Sender: TObject);
begin
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('UPDATE s_tovar SET izm1=2,kol_in=1 WHERE izm_id in (3,4,5)');
  dms.main_link.ExecSQL;
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('UPDATE s_tovar SET izm1=7 WHERE izm_id = 11');
  dms.main_link.ExecSQL;
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('UPDATE s_tovar SET izm1=8 WHERE izm_id = 12');
  dms.main_link.ExecSQL;
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('UPDATE s_tovar SET izm1=3 WHERE izm_id = 13');
  dms.main_link.ExecSQL;
  ShowMessage('Sabr uchun rahmat');
end;

procedure Thatolar.RzBitBtn2Click(Sender: TObject);
begin
  dms.main_link.Close;dms.main_link.SQL.Clear;
  dms.main_link.SQL.Add('ALTER TABLE ombor CHANGE id id BIGINT(20) NULL DEFAULT NULL AUTO_INCREMENT, add PRIMARY KEY (id);');
  dms.main_link.ExecSQL;
  ShowMessage('site qoldiq ok');
